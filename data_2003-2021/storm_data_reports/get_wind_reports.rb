...
wind_reports_csv_out_path = ARGV[0] || "wind_reports.csv"
...
wind_reports_file = open(wind_reports_csv_out_path, "w")
...
last_storm_events_database_report_time = Time.new(START_YEAR)
...
(START_YEAR..STOP_YEAR).each do |year|
  ...
  wind_rows.map! do |row|
    last_storm_events_database_report_time = [begin_end_times(row)[1], last_storm_events_database_report_time].max
