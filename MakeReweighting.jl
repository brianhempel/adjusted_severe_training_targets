...
function find_reweighting_factor(reports, seconds_to_period_i, target_count)
...
function do_it(prefix, reports_gridded, seconds_to_period_i, target_counts_gridded)
  open(joinpath(@__DIR__, "out", "$(prefix)_reweighting_grid_130_cropped.csv"), "w") do f
    ...
    for ((lat, lon), pt_reports, target_count, edge_factor) in zip(latlons, reports_gridded, target_counts_gridded, edge_factors)
      factor = find_reweighting_factor(pt_reports, seconds_to_period_i, target_count / edge_factor)
      println(f, "$lat,$lon,$factor")
    end
  end
end
